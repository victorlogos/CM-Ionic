...
export class Orders {
    ...
    cancelOrder(id) {
        this.service.updateOrder({
            "status": "cancelled"
        }, id).then((results) => this.handleCancelOrder(results));
    }
    handleCancelOrder(results) {
        this.functions.showAlert("success", "order has been cancelled");
        this.service.getOrders(this.filter).then((results) => this.orders = results);
    }
    reOrder(id) {
        this.service.updateOrder({
            "status": "pending"
        }, id).then((results) => this.handleReOrder(results));
    }
